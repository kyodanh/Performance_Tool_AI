...
/**
 * Upstream's "Generated by Grafana k6 Studio" wording feeds Grafana's cloud
 * usage reporting; LoadPilot scripts deliberately don't claim to be upstream's.
 */
export function generateScriptHeader(wizardUsed: boolean) {
  const header = `Generated by LoadPilot (${__APP_VERSION__}) on ${new Date().toISOString()}`

  return wizardUsed ? `${header} (test-setup-wizard)` : header
}
